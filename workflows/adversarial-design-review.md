...
1. **Open a separate critic context** — a fresh subagent or session that did not write the code.
   Give it the outcome + the artifact (screenshots and/or the rendered route), not the builder's
   commentary.
2. **Prompt it to refute, not to praise:** "Find the strongest reasons this fails the outcome and
...
3. **Back judgment with objective checks** on changed files so taste and defects stay separable:
...
- Screenshot(s) of the artifact the critic actually judged.
- The three checks' output.
- Each critic finding tied to a rubric row or a `file:line` — no free-floating "feels off."

## Output

A filled [`grader-report-template.md`](../templates/grader-report-template.md) with a verdict:
...
- No outcome is defined — you cannot judge against nothing. Write the outcome first (one pass of
  [`outcome-template.md`](../templates/outcome-template.md)), then review.
