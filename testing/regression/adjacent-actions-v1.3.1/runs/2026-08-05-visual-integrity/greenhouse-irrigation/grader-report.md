...
> Candidate rerun note, 2026-08-05: this prior independent grade is retained because the current
> candidate matrix is 8/8 bit-for-bit identical to the prior passing evidence. The candidate verifier
...
Rendered hard-gate verification from `evidence.json`: axe available; zero serious/critical violations; no horizontal overflow; no main-landmark failures; no live-region failures; no touch targets under 48px; CLS available for all eight captures with maximum `0`; no CLS failures; all requested states confirmed rendered.
